...
Class Action {
	...
	// Method to get Tenant ID by Tenant Name
    public function getTenantIdByName($tenantName) {
        // Query the database to retrieve the tenant ID based on the provided tenant name
        $query = $this->db->query("SELECT id FROM tenants WHERE CONCAT(lastname, ', ', firstname, ' ', middlename) = '$tenantName'");
    ...
    public function save_mpesa_payment() {
        extract($_POST); // Extract POST data
        $data = ""; // Initialize data variable

        // Get the tenant ID based on the provided tenant name
        $tenant_id = $this->getTenantIdByName($tenantName);
        ...
        if ($tenant_id) {
            ...
            // Add the retrieved tenant ID to the data string
            ...
            // Return success (1) if the operation was successful
            if ($save) {
                return 1;
            }
        } else {
            return 0; // Tenant ID not found for the provided name
        }
    }
